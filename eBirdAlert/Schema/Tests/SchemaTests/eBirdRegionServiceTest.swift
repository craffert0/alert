...
struct eBirdRegionServiceTest {
    ...
    @Test func getRegionsAtCross() throws {
        let result = service.getRegions(at: .hudsonRiver)
        try #require(result.count == 2)
        #expect(result[0].result == "Hudson, New Jersey, United States")
        #expect(result[1].result == "New York, New York, United States")
    }

    @Test func getRegionVariations() {
        #expect(service.getRegion(at: .prospectPark) == .kings)
        #expect(service.getRegion(at: .silvercup) == .nyc)
        #expect(service.getRegion(at: .hudsonRiver)?.code == "US-NJ-017")
    }
}
